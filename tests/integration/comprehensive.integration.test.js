...
describe.skip("Comprehensive Integration Tests", () => {
  ...
  describe("Workflow: Complete Product Management", () => {
    ...
    it("should create a complete workflow: grupo -> fornecedor -> produto -> movimentacao", async () => {
      // 1. Create Grupo
      const grupoResponse = await helper
        .request()
        .post("/api/grupos")
      ...
      // 2. Create Fornecedor
      const fornecedorResponse = await helper
        .request()
      ...
      // 3. Create Produto
      const produtoResponse = await helper
        .request()
        .post("/produtos")
        .set("Authorization", `Bearer ${userToken}`)
        .send({
      ...
      const movimentacaoResponse = await helper
        .request()
    ...
    it("should list all entities created in workflow", async () => {
      // List Grupos
      const gruposResponse = await helper
        .request()
        .get("/api/grupos")
        .set("Authorization", `Bearer ${userToken}`);
      ...
      // List Fornecedores
      const fornecedoresResponse = await helper
        .request()
      ...
      // List Produtos
      const produtosResponse = await helper
        .request()
        .get("/produtos")
        .set("Authorization", `Bearer ${userToken}`);
      ...
      // List Movimentacoes
      const movimentacoesResponse = await helper
        .request()
