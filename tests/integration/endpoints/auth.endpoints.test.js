...
describe("Auth Endpoints Integration Tests", () => {
  ...
  describe("POST /recuperar-senha", () => {
    it("should accept email for password recovery", async () => {
      const response = await request(app).post("/recuperar-senha").send({
  ...
  describe("POST /redefinir-senha/token", () => {
    it("should validate token-based password reset", async () => {
      const response = await request(app).post("/redefinir-senha/token").send({
        token: "fake_token",
  ...
  describe("POST /redefinir-senha/codigo", () => {
    it("should validate code-based password reset", async () => {
      const response = await request(app).post("/redefinir-senha/codigo").send({
        email: "admin@teste.com",
        codigo: "123456",
        novaSenha: "newpassword123",
        confirmarSenha: "newpassword123",
  ...
  describe("POST /revoke (Protected)", () => {
    beforeEach(async () => {
      // Obter token válido
      const loginResponse = await request(app).post("/login").send({
        matricula: "ADM0001",
