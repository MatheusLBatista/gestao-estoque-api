  ...
  async createDefaultGroups() {
    try {
      ...
      const grupos = [
        ...
        {
          nome: "estoquista",
          descricao: "Responsáveis pelo estoque",
          ativo: true,
          permissoes: [],
        },
        {
          nome: "auditor",
          descricao: "Auditores do sistema",
          ativo: true,
          permissoes: [],
        },
      ];
  ...
  async login(matricula = "ADM0001", senha = "admin123") {
    try {
      ...
      if (response.status !== 200) {
        ...
        throw new Error(
          `Login falhou: ${response.body?.message || "Erro desconhecido"}`
        );
      }
      ...
      if (response.status !== 201) {
        ...
        throw new Error(
          `Falha ao criar usuário: ${
            response.body?.message || "Erro desconhecido"
          }`
        );
      }
