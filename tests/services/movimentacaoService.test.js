...
describe("MovimentacaoService", () => {
  ...
  describe("listarMovimentacoes", () => {
    ...
    it("should handle repository errors", async () => {
      ...
      await expect(service.listarMovimentacoes(mockReq)).rejects.toThrow(
        "Repository error"
      );
    });
  });
  ...
  describe("buscarMovimentacaoPorID", () => {
    it("should return movement by ID successfully", async () => {
      ...
      expect(mockRepository.buscarMovimentacaoPorID).toHaveBeenCalledWith(
        mockId
      );
    ...
    it("should throw error for invalid ID", async () => {
      ...
      await expect(service.buscarMovimentacaoPorID(invalidId)).rejects.toThrow(
    ...
    it("should throw error when movement not found", async () => {
      ...
      await expect(service.buscarMovimentacaoPorID(mockId)).rejects.toThrow(
  ...
  describe("buscarMovimentacoesPorTipo", () => {
    ...
    it("should throw error for invalid type", async () => {
      await expect(
        service.buscarMovimentacoesPorTipo("invalid")
      ).rejects.toThrow("Tipo de movimentação inválido");
      await expect(service.buscarMovimentacoesPorTipo("")).rejects.toThrow(
        "Tipo de movimentação inválido"
      );
      await expect(service.buscarMovimentacoesPorTipo(null)).rejects.toThrow(
        "Tipo de movimentação inválido"
      );
    });
  ...
  describe("buscarMovimentacoesPorPeriodo", () => {
    it("should return movements by period successfully", async () => {
      ...
      const result = await service.buscarMovimentacoesPorPeriodo(
        "2023-01-01",
        "2023-01-31",
        1,
        10
    ...
    it("should throw error for invalid date format", async () => {
      await expect(
        service.buscarMovimentacoesPorPeriodo("invalid-date", "2023-01-31")
      ).rejects.toThrow("Formato de data inválido");
    });

    it("should throw error when start date is after end date", async () => {
      await expect(
        service.buscarMovimentacoesPorPeriodo("2023-01-31", "2023-01-01")
  ...
  describe("buscarMovimentacoesPorProduto", () => {
    it("should return movements by product successfully", async () => {
      ...
      const result = await service.buscarMovimentacoesPorProduto(
        "produto123",
        1,
        10
      );
    ...
    it("should throw error for empty product identifier", async () => {
      await expect(service.buscarMovimentacoesPorProduto("")).rejects.toThrow(
        "Identificador do produto é obrigatório"
      );
      await expect(service.buscarMovimentacoesPorProduto(null)).rejects.toThrow(
        "Identificador do produto é obrigatório"
      );
      await expect(
        service.buscarMovimentacoesPorProduto("   ")
      ).rejects.toThrow("Identificador do produto é obrigatório");
    });
  });
  ...
  describe("buscarMovimentacoesPorUsuario", () => {
    it("should return movements by user successfully", async () => {
      ...
      const result = await service.buscarMovimentacoesPorUsuario(
        "user123",
        1,
        10
      );
    ...
    it("should throw error for empty user identifier", async () => {
      await expect(service.buscarMovimentacoesPorUsuario("")).rejects.toThrow(
        "Identificador do usuário é obrigatório"
      );
      await expect(service.buscarMovimentacoesPorUsuario(null)).rejects.toThrow(
        "Identificador do usuário é obrigatório"
      );
      await expect(
        service.buscarMovimentacoesPorUsuario("   ")
      ).rejects.toThrow("Identificador do usuário é obrigatório");
    });
  });
  ...
  describe("cadastrarMovimentacao", () => {
    ...
    it("should register entrada movement successfully", async () => {
      const dadosMovimentacao = {
        tipo: "entrada",
        produtos: [{ _id: "product123", quantidade_produtos: 10 }],
      };
      ...
      expect(mockProdutoService.buscarProdutoPorID).toHaveBeenCalledWith(
        "product123"
      );
      expect(mockProdutoService.atualizarProduto).toHaveBeenCalledWith(
        "product123",
        {
    ...
    it("should throw error when no products provided", async () => {
      ...
      await expect(
        service.cadastrarMovimentacao(dadosMovimentacao)
    ...
    it("should throw error for insufficient stock on saida", async () => {
      const dadosMovimentacao = {
        tipo: "saida",
        produtos: [{ _id: "product123", quantidade_produtos: 150 }],
      };
    ...
    it("should throw error when product not found", async () => {
      const dadosMovimentacao = {
        tipo: "entrada",
        produtos: [{ _id: "nonexistent", quantidade_produtos: 10 }],
      };
  ...
  describe("atualizarMovimentacao", () => {
    ...
    it("should throw error for invalid ID", async () => {
      ...
      await expect(
        service.atualizarMovimentacao("invalid", {})
    ...
    it("should throw error when movement not found", async () => {
      ...
      await expect(
        service.atualizarMovimentacao("movement123", {})
    ...
    it("should throw error for movements older than 24 hours", async () => {
      ...
      const dadosAtualizacao = {
        produtos: [{ _id: "product123", quantidade_produtos: 5 }],
      };
      ...
      await expect(
        service.atualizarMovimentacao("movement123", dadosAtualizacao)
      ).rejects.toThrow(
  ...
  describe("deletarMovimentacao", () => {
    ...
    it("should throw error for invalid ID", async () => {
      ...
      await expect(service.deletarMovimentacao("invalid")).rejects.toThrow(
        "ID da movimentação inválido"
      );
    });
    ...
    it("should throw error when movement not found", async () => {
      ...
      await expect(service.deletarMovimentacao("movement123")).rejects.toThrow(
        "Movimentação não encontrada"
      );
    });
    ...
    it("should handle product errors gracefully", async () => {
      ...
      mongoose.Types.ObjectId.isValid.mockReturnValue(true);
      mockRepository.buscarMovimentacaoPorID.mockResolvedValue(mockMovement);
      mockProdutoService.buscarProdutoPorID.mockRejectedValue(
        new Error("Product error")
      );
      mockRepository.deletarMovimentacao.mockResolvedValue({ deletedCount: 1 });
  ...
  describe("filtrarMovimentacoesAvancado", () => {
    it("should filter movements successfully", async () => {
      ...
      const result = await service.filtrarMovimentacoesAvancado(
        filtros,
        opcoesPaginacao
      );

      expect(mockRepository.filtrarMovimentacoesAvancado).toHaveBeenCalledWith(
        filtros,
        opcoesPaginacao
      );
      expect(result).toEqual(mockResult);
    ...
    it("should validate date filters", async () => {
      ...
      await expect(
        service.filtrarMovimentacoesAvancado(filtros)
      ).rejects.toThrow("Formato de data inválido");
    });
  ...
  describe("desativarMovimentacao", () => {
    ...
    it("should throw error for invalid ID", async () => {
      ...
      await expect(service.desativarMovimentacao("invalid")).rejects.toThrow(
        "ID da movimentacao inválido"
      );
    });
  });
  ...
  describe("reativarMovimentacao", () => {
    ...
    it("should throw error for invalid ID", async () => {
      ...
      await expect(service.reativarMovimentacao("invalid")).rejects.toThrow(
        "ID da movimentacao inválido"
      );
    });
  });
