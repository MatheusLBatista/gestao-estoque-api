...
describe("MovimentacaoFilterBuilder", () => {
  ...
  describe("comPeriodo", () => {
    it("should filter by date period", () => {
      ...
      expect(filterBuilder.filtros.data_movimentacao.$gte).toEqual(
        new Date(dataInicio)
      );
      expect(filterBuilder.filtros.data_movimentacao.$lte.getHours()).toBe(23);
      expect(filterBuilder.filtros.data_movimentacao.$lte.getMinutes()).toBe(
        59
      );
    });
  ...
  describe("comProdutoId", () => {
    it("should filter by valid produto ID", () => {
      ...
      expect(filterBuilder.filtros["produtos._id"]).toBe(validId.toString());
    });

    it("should ignore invalid ObjectId", () => {
      filterBuilder.comProdutoId("invalid-id");
      expect(filterBuilder.filtros["produtos._id"]).toBeUndefined();
    });
  ...
  describe("comQuantidadeMinima", () => {
    ...
    it("should ignore invalid numbers", () => {
      ...
      expect(
        filterBuilder.filtros["produtos.quantidade_produtos"]
      ).toBeUndefined();
    });
  ...
  describe("comDataApos", () => {
    ...
    it("should combine with existing date filters", () => {
      ...
      expect(filterBuilder.filtros.data_movimentacao.$lte).toEqual(
        new Date("2023-01-31")
      );
    });
  ...
  describe("escapeRegex", () => {
    it("should escape special regex characters", () => {
      const input =
        "Test (Special) [Characters] {More} +Plus ?Question *Star .Dot ^Caret $Dollar |Pipe #Hash";
      const expected =
        "Test\\ \\(Special\\)\\ \\[Characters\\]\\ \\{More\\}\\ \\+Plus\\ \\?Question\\ \\*Star\\ \\.Dot\\ \\^Caret\\ \\$Dollar\\ \\|Pipe\\ \\#Hash";
  ...
  describe("Method chaining", () => {
    it("should allow chaining multiple filters", () => {
      ...
      expect(filters["produtos.quantidade_produtos"]).toEqual({
        $gte: 10,
        $lte: 100,
      });
    });
