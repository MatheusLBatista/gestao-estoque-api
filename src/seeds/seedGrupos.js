...
async function seedGrupos() {
  try {
    ...
    // Deletar todos os grupos existentes para garantir atualização
    const gruposExistentes = await Grupo.countDocuments();
    if (gruposExistentes > 0) {
      console.log(`🗑️  Deletando ${gruposExistentes} grupos antigos...`);
      await Grupo.deleteMany({});
      console.log("✅ Grupos antigos removidos com sucesso!");
    }

    const todasAsRotas = await import("../models/Rotas.js").then(
      (module) => module.default
    );
    const rotasExistentes = await todasAsRotas.find({});

    console.log(
      `📋 Encontradas ${rotasExistentes.length} rotas no sistema para sincronização de permissões`
    );
    ...
    const permissoesGerenteCompletas = rotasExistentes
      ...
      .map((rota) => ({
        ...
        enviar: ["produtos", "movimentacoes", "fornecedores"].includes(
          rota.rota
        ),
        substituir: ["produtos", "movimentacoes", "fornecedores"].includes(
          rota.rota
        ),
        modificar: ["produtos", "movimentacoes", "fornecedores"].includes(
          rota.rota
        ),
        excluir: false,
      }));
    ...
    const permissoesEstoquistaCompletas = rotasExistentes
      .filter((rota) =>
        ["produtos", "fornecedores", "movimentacoes", "dashboard"].includes(
          rota.rota
        )
      )
      .map((rota) => ({
    ...
    const indexAdmin = gruposAtualizados.findIndex(
      (g) => g.nome === "Administradores"
    );
    if (indexAdmin !== -1) {
      gruposAtualizados[indexAdmin].permissoes = permissoesAdminCompletas;
      gruposAtualizados[
        indexAdmin
      ].descricao = `Grupo com acesso ABSOLUTO - ${permissoesAdminCompletas.length} rotas com TODAS as permissões (GET, POST, PUT, PATCH, DELETE)`;
      console.log(
        `Grupo Administradores configurado com ${permissoesAdminCompletas.length} permissões completas`
      );
    }

    const indexGerente = gruposAtualizados.findIndex(
      (g) => g.nome === "Gerentes"
    );
    if (indexGerente !== -1) {
      ...
      gruposAtualizados[
        indexGerente
      ].descricao = `Grupo com acesso de gerenciamento - ${permissoesGerenteCompletas.length} rotas com permissões limitadas`;
      console.log(
        `Grupo Gerentes configurado com ${permissoesGerenteCompletas.length} permissões limitadas`
      );
    }

    const indexEstoquista = gruposAtualizados.findIndex(
      (g) => g.nome === "Estoquistas"
    );
    if (indexEstoquista !== -1) {
      gruposAtualizados[indexEstoquista].permissoes =
        permissoesEstoquistaCompletas;
      gruposAtualizados[
        indexEstoquista
      ].descricao = `Grupo com acesso básico - ${permissoesEstoquistaCompletas.length} rotas com permissões básicas`;
      console.log(
        `Grupo Estoquistas configurado com ${permissoesEstoquistaCompletas.length} permissões básicas`
      );
    }
    ...
    console.log(
      "👥 Grupos criados:",
      gruposInseridos.map((g) => g.nome).join(", ")
    );

    const grupoAdminCriado = gruposInseridos.find(
      (g) => g.nome === "Administradores"
    );
    const grupoGerenteCriado = gruposInseridos.find(
      (g) => g.nome === "Gerentes"
    );
    const grupoEstoquistaCriado = gruposInseridos.find(
      (g) => g.nome === "Estoquistas"
    );
    ...
    if (grupoAdminCriado) {
      ...
      console.log(
        "   📋 Rotas com acesso total:",
        grupoAdminCriado.permissoes.map((p) => p.rota).join(", ")
      );
    }
    ...
    if (grupoGerenteCriado) {
      ...
      console.log(
        "   📋 Rotas com acesso gerencial:",
        grupoGerenteCriado.permissoes.map((p) => p.rota).join(", ")
      );
    }
    ...
    if (grupoEstoquistaCriado) {
      ...
      console.log(
        "   📋 Rotas com acesso básico:",
        grupoEstoquistaCriado.permissoes.map((p) => p.rota).join(", ")
      );
    }
