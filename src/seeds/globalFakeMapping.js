...
function validateModelMapping(model, mapping) {
  ...
  const invalidFields = mappingFields.filter(
    (field) => !schemaFields.includes(field)
  );
  if (invalidFields.length > 0) {
    console.warn(
      `Aviso: Campos inválidos no mapeamento: ${invalidFields.join(", ")}`
    );
  }
...
function getGlobalFakeMapping() {
  const fakeMapping = {
    usuario: {
      ...
      perfil: () =>
        fakerPT_BR.helpers.arrayElement([
          "administrador",
          "gerente",
          "vendedor",
          "estoquista",
    ...
    produto: {
      ...
      preco: () =>
        parseFloat(fakerPT_BR.commerce.price({ min: 15, max: 10000 })),
      marca: () => fakerPT_BR.company.name(),
      custo: () => parseFloat(fakerPT_BR.commerce.price({ min: 5, max: 8000 })),
      categoria: function () {
        if (this.preco >= 1001.0) {
          return "A";
        } else if (this.preco >= 500.0) {
          return "B";
        } else {
          return "C";
        }
    ...
    movimentacao: {
      ...
      produtos: {
        ...
        custo: () =>
          parseFloat(fakerPT_BR.commerce.price({ min: 50, max: 200 })),
        id_fornecedor: () => fakerPT_BR.number.int({ min: 1, max: 100 }),
        nome_fornecedor: () => fakerPT_BR.company.name(),
      },
    },
