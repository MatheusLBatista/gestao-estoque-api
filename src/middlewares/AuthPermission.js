...
class AuthPermission {
  ...
  async handle(req, res, next) {
    try {
      ...
      // Para URLs como /produtos, pega 'produtos' (segundo segmento)
      const rotaReq =
      ...
      if (!hasPermission) {
        ...
        console.warn(
          `Acesso negado para usuário ${userInfo} na rota ${rotaReq} com método ${req.method}`
        );
      ...
      console.log(
        `Autorização concedida para usuário ${
          decoded.matricula || userId
        } na rota ${rotaReq} com método ${req.method}`
      );
