...
export const UsuarioSchema = z.object({
  nome_usuario: z
    ...
    .regex(
      /^[A-Za-zÀ-ÿ\s]+$/,
      "Nome do usuário deve possuir pelo menos 3 caracteres, não pode conter caracteres especiais ou numeros. Apenas letras."
    ),
  email: z
    .string()
  ...
  telefone: z
    ...
    .max(15, "Telefone deve estar no formato (XX) XXXX-XXXX ou (XX) XXXXX-XXXX")
    .regex(
      /^\(\d{2}\)\s\d{4,5}-\d{4}$/,
  ...
  perfil: z.enum(["administrador", "gerente", "vendedor", "estoquista"], {
    errorMap: () => ({
      message:
        "Perfil deve ser um dos seguintes: administrador, gerente, vendedor ou estoquista.",
    }),
  }),
  matricula: z
    .string()
  ...
  grupos: z
    .array(
      z
        .string()
        .regex(/^[0-9a-fA-F]{24}$/, "ID do grupo deve ser um ObjectId válido")
    )
    .optional()
    .default([])
