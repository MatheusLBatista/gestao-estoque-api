...
class ProdutoRepository {
  ...
  async listarProdutos(req) {
    ...
    const stats = {
      total_produtos: resultado.totalDocs || 0,
    ...
    resultado.docs.forEach((prod) => {
      stats.valor_total_estoque += (prod.preco || 0) * (prod.estoque || 0);
      stats.custo_total_estoque += (prod.custo || 0) * (prod.estoque || 0);
      if (prod.estoque <= (prod.estoque_min || 0)) {
        stats.produtos_estoque_baixo++;
      }
    });

    // Arredondamento
    stats.valor_total_estoque = parseFloat(
      stats.valor_total_estoque.toFixed(2)
    );
    stats.custo_total_estoque = parseFloat(
      stats.custo_total_estoque.toFixed(2)
    );
  ...
  async buscarProdutoPorID(id) {
    const produto = await this.model
      .findById(id)
      .populate("fornecedores", "_id nome_fornecedor cnpj email");
    if (!produto) {
      throw new CustomError({
  ...
  async buscarPorNome(nome) {
    let query = this.model.find({
      nome_produto: { $regex: nome, $options: "i" },
    });
  ...
  async atualizarProduto(id, dadosProduto) {
    ...
    const produto = await this.model.findByIdAndUpdate(id, dadosProduto, {
      new: true,
      runValidators: true,
    });
